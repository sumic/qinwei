    ...
    meTables = function (options) {
      ...
      this.updateAll = function () {
        ...
        if (data) {
          ...
          swal(meTables.getLanguage("error"), meTables.getLanguage("noSelect"),"error"); 
        }
      ...
      this.delete = function (row) {
        ...
        var swalText;
        if(pkTitle !=''){
          swalText = meTables.getLanguage("confirm").replace("_LENGTH_", "")+"("+pkTitle+":"+self.table.data()[row][self.options.pk]+")";
        }else{
          swalText = meTables.getLanguage("confirm").replace("_LENGTH_", "");
        }
        swal({ 
          title: meTables.getLanguage("confirmOperation"), 
          text: swalText, 
          ...
          confirmButtonText: meTables.getLanguage("determine"), 
          cancelButtonText: meTables.getLanguage("cancel"),
      ...
      this.deleteAll = function () {
        ...
        // 数据为空提醒
        if (data.length < 1) {
          swal(meTables.getLanguage("error"), meTables.getLanguage("noSelect"),"error"); return false;
        }
        ...
        swal({ 
          title: meTables.getLanguage("confirmOperation"), 
          text: meTables.getLanguage("confirm").replace("_LENGTH_", data.length)+"(ids:"+data.join(',')+")", 
          type: "warning",
          showCancelButton: true, 
          confirmButtonColor: "#DD6B55",
          confirmButtonText: meTables.getLanguage("determine"), 
        ...
        deferred.fail(function (result) {
          if (result) {
            try {
              ...
              $.notify({icon:'fa fa-bell',message:"<strong>"+meTables.getLanguage("sServerError")+"</<strong>"})
            }
          } else {
            $.notify({icon:'fa fa-bell',message:"<strong>"+meTables.getLanguage("sExport")+"</<strong>"})
          }
        }).always(function () {
        ...
        this.options.table.ajax = {
          ...
          dataSrc: function (json) {
            ...
            if (!self.options.isSuccess(json) && json.draw === undefined) {
              $.notify({icon:'fa fa-bell',message:"<strong>"+self.options.getMessage(json)+"</<strong>"});
              json.draw = 1
              json.recordsTotal = 0
