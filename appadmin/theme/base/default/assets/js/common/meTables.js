    ...
    meTables.fn = meTables.prototype = {
        ...
        updateAll: function() {
            ...
            if (row) {
                ...
                return swal({ 
                	  title: this.getLanguage("notice"), 
                	  text: this.getLanguage("noSelect"), 
                	  type: "warning",
        ...
        delete: function(row, child) {
            ...
            //查找主键对应的标题
            var titleColumns = self.options.table.aoColumns;
            var pkTitle = '';
            var pkName  = self.options.pk;
            jQuery.each(titleColumns, function(i, val) {  
            	if(val.data == pkName)pkTitle = val.title; 
            });
            var swalText;
            if(pkTitle !=''){
            	swalText = this.getLanguage("confirm").replace("_LENGTH_", "")+"("+pkTitle+":"+self.table.data()[row][self.options.pk]+")";
            }else{
            	swalText = this.getLanguage("confirm").replace("_LENGTH_", "");
            }
            swal({ 
          	  title: self.getLanguage("confirmOperation"), 
          	  text: swalText, 
          	  ...
          	  confirmButtonText: self.getLanguage("determine"), 
          	  cancelButtonText: self.getLanguage("cancel"),
          	  closeOnConfirm: false, 
          	  closeOnCancel: false	
          	},
          	function(isConfirm){ 
          	  if (isConfirm) { 
          	    ...
          	    swal("取消！", self.getLanguage("cancelOperation"),"error"); 
          	  } 
          	});
        ...
        deleteAll: function() {
            ...
            swal({ 
            	  title: self.getLanguage("confirmOperation"), 
            	  text: this.getLanguage("confirm").replace("_LENGTH_", data.length)+"(ids:"+data.join(',')+")", 
            	  type: "warning",
            	  showCancelButton: true, 
            	  confirmButtonColor: "#DD6B55",
            	  confirmButtonText: self.getLanguage("determine"), 
                ...
                if (data) {
                    ...
                    if (typeof self.beforeSave !== 'function' || self.beforeSave(data, child)) {
                        ...
                        meTables.ajax({
                            ...
                            if (json.errCode === 0) {
                                // 执行成功之后的数据处理
                            	swal(self.getLanguage("operationSuccess"), json.errMsg,"success");
                            	...
                            	swal(self.getLanguage("operationFail"), json.errMsg,"error");
                            }
                        });
                    }
                }
            } else {
                sweetAlert(self.getLanguage("operationError"), "出错了！","error");
            ...
            deferred.fail(function(result) {
                if (result) {
                    try {
                        ...
                        $.notify({icon:'fa fa-bell',message:"<strong>"+result.errMsg+"</<strong>"});
                    } catch (e) {
                        $.notify({icon:'fa fa-bell',message:"<strong>"+self.getLanguage("sServerError")+"</<strong>"});
                    }
                } else {
                    $.notify({icon:'fa fa-bell',message:"<strong>"+self.getLanguage("sExport")+"</<strong>"},{type:'info'});
                }
    ...
    meTables.extend({
        ...
        ajax: function (params) {
            ...
            return $.ajax(params).always(function () {
                ...
                $.notify({icon:'fa fa-bell',message:"<strong>"+meTables.fn.getLanguage("sServerError")+"</<strong>"});
            });
        },
        ...
        // 是否
        yesnoString:function(td, data) {
            $(td).html('<span class="label label-' + (parseInt(data) === 1 ? 'success">是' : 'disable">否') + '</span>');
        },
