...
class Tree extends Service
{
    ...
    /**
     * 设置数据信息
     *['data'=>$array ,'parentIdName'=>'pid','childrenName' = 'childname']
     * @param $array
     */
    public function setParam($param)
    {
        if(!is_array($param)){
            Yii::$service->helper->errors->add('参数设置不正确');
            return false;
        }
        if(isset($param['data']) && !empty($param['data'])){
            $this->array = $param['data'];
        }
        if (isset($param['parentIdName']) && !empty($param['parentIdName'])){
            $this->parentIdName = $param['parentIdName'];
        }
        if (isset($param['childrenName']) && !empty($param['childrenName'])){
            $this->childrenName = $param['childrenName'];
        }
        $this->html = '';
        return $this;
    }
     ...
     * 生成带分隔符的树型结构数组
     * @param integer $id 表示获得这个ID下的所有子级
     * @param integer $name 字段名称
     * @return array
     */
    public function getTreeArraySpec($id,$name, $prefix = '',&$dd=[])
    {
        $number = 1;
        // 一级栏目
        $child = $this->getChild($id);
        if (is_array($child)) {
            ...
            foreach ($child as $key => $value) {
                ...
                // 确定使用的处理方式
                // 建议使用函数数据
                $dd[$value['id']] = $value['extend_space'].$value[$name];
                $this->getTreeArraySpec(
                    $value['id'],
                    $name,
                    $prefix . $arrSeparator['suffix'] . $this->space,
                    $dd
